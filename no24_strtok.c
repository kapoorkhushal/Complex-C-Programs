...
int main(int argc,char* argv[])
{
	...
	while(NULL != fgets(str,1000,input))
	{
		duplicate = strtok(str,delimit);
		while(NULL != duplicate)
		{
			int flag = 0;
			while(NULL != fgets(check,10,output))
			{
				
				if(0 == strncmp(duplicate,check,strlen(duplicate)))
				{
					flag = 1;
					break;
				}
			...
			if(0  == flag)
			{
				fputs(duplicate,output);
				fputs("\n",output);
			}
			rewind(output);
			duplicate = strtok(NULL,delimit);
		}
	}
	rewind(output);
	printf("\nOUTPUT FILE ::\n");
